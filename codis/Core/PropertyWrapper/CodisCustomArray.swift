...
/// Codis自定义类型数组配置包装器，专门用于[CodisCustomLimitType]类型
/// 提供@propertyWrapper语法糖，简化自定义类型数组的存取操作
/// 支持自动序列化和反序列化，底层使用JSON编解码
@propertyWrapper
public struct CodisCustomArray<T: CodisCustomLimitType> {
    /// 配置的key值，用于唯一标识该配置项
